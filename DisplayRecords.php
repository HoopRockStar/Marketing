...
function showContent($title, $category, $id) {
   ...
   $sql = "
             SELECT TELEPHONE_NUMBER as 'Phone:', FAX_NUMBER as 'Fax:', EMAIL as 'Email:', PRIORITY as 'Priority:', REFERAL as 'Referal:', PERMISSION as 'Permission:', NEWSLETTER as 'Newsletter:', TGT_DBF as 'Target Database:', HISTORY as 'History:', NO_CONTACT as 'No Contact?', DATE_ENTERED as 'Date Entered:', X_FERED as 'Date Transfered:'
             FROM $category
             WHERE ID='$id';
             ";
   ...
   $date = mysql_result($result, 0, 10);
   $_SESSION["date"] = $date;
   $transfered = mysql_result($result, 0, 11);
   $_SESSION["transfered"] = $transfered;
