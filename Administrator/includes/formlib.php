...
class FormLib extends FormVerifier {
	...
	/**
	 * Returns the HTML to create a multi-line text-input control.
	 *
	 * @param $name The name of the form control.
	 * @param $height The height of the form control
	 * @param $size The number of columns in the text input.
	 * @param $value The text to initially display.
	 * @param $other Other attributes added to the control.
	 */
	function makeTextInputwithHeight($name, $size = 0, $height="", $value = "", $other = "") {
	    $value = $this->getValue($name, $value);
	    $html = "<input type=\"text\" name=\"$name\" value=\"$value\"";
	    if ($size) $html .= " size=\"$size\"";
     ...
     * Note that password values are not retained on error. If you want
     * to retain the value on error, then call the function like this:
     * echo $f->makePassword($name, $size, $f->getValue($name);
     *
     * @param $name The name of the form control.
     * @param $size The number of columns in the text input.
